...
class appikorn_date_package_data extends StatefulWidget {
  const appikorn_date_package_data({
  ...
  final appikorn_date_picker_config config;
  ...
  @override
  State<appikorn_date_package_data> createState() => _appikorn_date_package_dataState();
}

class _appikorn_date_package_dataState extends State<appikorn_date_package_data> {
  ...
  @override
  void didUpdateWidget(covariant appikorn_date_package_data oldWidget) {
    var isValueSame = oldWidget.initialValue.length == widget.initialValue.length;

    if (isValueSame) {
      for (var i = 0; i < oldWidget.initialValue.length; i++) {
        var isSame = (oldWidget.initialValue[i] == null && widget.initialValue[i] == null) || DateUtils.isSameDay(oldWidget.initialValue[i], widget.initialValue[i]);
        if (!isSame) {
          isValueSame = false;
          break;
        }
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            if ((widget.config.gapBetweenCalendarAndButtons ?? 0) > 0) SizedBox(width: widget.config.gapBetweenCalendarAndButtons),
...
class appikorn_date_picker_config extends CalendarDatePicker2Config {
  appikorn_date_picker_config({
    CalendarDatePicker2Type? calendarType,
    DateTime? firstDate,
    DateTime? lastDate,
    DateTime? currentDate,
  ...
  @override
  appikorn_date_picker_config copyWith({
    CalendarDatePicker2Type? calendarType,
    DateTime? firstDate,
    DateTime? lastDate,
    DateTime? currentDate,
    ...
    return appikorn_date_picker_config(
      calendarType: calendarType ?? this.calendarType,
      firstDate: firstDate ?? this.firstDate,
      lastDate: lastDate ?? this.lastDate,
      currentDate: currentDate ?? this.currentDate,
...
class CalendarDatePicker2 extends StatefulWidget {
  CalendarDatePicker2({
    ...
    if (config.calendarType == CalendarDatePicker2Type.range && initialValue.length > 1) {
      final isRangePickerValueValid =
          initialValue[0] == null ? (initialValue[1] != null ? invalid : valid) : (initialValue[1] != null ? (initialValue[1]!.isBefore(initialValue[0]!) ? invalid : valid) : valid);
...
class _CalendarDatePicker2State extends State<CalendarDatePicker2> {
  ...
  void initState() {
    ...
    final initialDate = widget.initialValue.isNotEmpty && widget.initialValue[0] != null ? DateTime(widget.initialValue[0]!.year, widget.initialValue[0]!.month) : DateUtils.dateOnly(DateTime.now());
  ...
  void _handleMonthChanged(DateTime date, {bool fromYearPicker = false}) {
    setState(() {
      ...
      if (fromYearPicker) {
        final selectedDatesInThisYear = widget.initialValue.where((d) => d?.year == date.year).toList()..sort((d1, d2) => d1!.compareTo(d2!));
  ...
  void _handleDayChanged(DateTime value) {
    ...
    setState(() {
      ...
      final isValueDifferent = widget.config.calendarType != CalendarDatePicker2Type.single || !DateUtils.isSameDay(selectedDates[0], _selectedDates.isNotEmpty ? _selectedDates[0] : null);
      if (isValueDifferent) {
          ...
          Flexible(
            child: Semantics(
              ...
              child: SizedBox(
                height: (widget.config.controlsHeight ?? _subHeaderHeight),
                child: InkWell(
                  onTap: widget.config.disableYearPicker == true ? null : widget.onTitlePressed,
                  child: Padding(
  ...
  Widget build(BuildContext context) {
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        final bool isDisabled = dayToBuild.isAfter(widget.config.lastDate) || dayToBuild.isBefore(widget.config.firstDate) || !(widget.config.selectableDayPredicate?.call(dayToBuild) ?? true);
...
class _YearPickerState extends State<YearPicker> {
  ...
  void initState() {
    super.initState();
    final scrollOffset = widget.selectedDates.isNotEmpty && widget.selectedDates[0] != null ? _scrollOffsetForYear(widget.selectedDates[0]!) : _scrollOffsetForYear(DateUtils.dateOnly(DateTime.now()));
  ...
  void didUpdateWidget(YearPicker oldWidget) {
    ...
    if (widget.selectedDates != oldWidget.selectedDates) {
      final scrollOffset =
          widget.selectedDates.isNotEmpty && widget.selectedDates[0] != null ? _scrollOffsetForYear(widget.selectedDates[0]!) : _scrollOffsetForYear(DateUtils.dateOnly(DateTime.now()));
